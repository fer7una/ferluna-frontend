...
import type { SiteData } from "../../domain/site/types";
...
export type AdminContentData = Pick<
  SiteData,
  "profile" | "visualSettings" | "sections" | "sectionItems" | "momentaryTabs" | "momentaryItems"
>;
...
export function normalizeSiteData(payload: Partial<SiteData>): SiteData {
  return {
    profile: payload.profile ?? fallbackSiteData.profile,
    visualSettings: normalizeVisualSettings(payload.visualSettings),
...
export function normalizeVisualSettings(value: unknown): SiteData["visualSettings"] {
  if (!value || typeof value !== "object") {
    return fallbackSiteData.visualSettings;
  }

  const settings = value as Partial<SiteData["visualSettings"]>;
  return {
    sectionOrbitDurationSeconds: positiveNumberOrFallback(
      settings.sectionOrbitDurationSeconds,
      fallbackSiteData.visualSettings.sectionOrbitDurationSeconds,
    ),
    momentaryOrbitDurationSeconds: positiveNumberOrFallback(
      settings.momentaryOrbitDurationSeconds,
      fallbackSiteData.visualSettings.momentaryOrbitDurationSeconds,
    ),
  };
}
